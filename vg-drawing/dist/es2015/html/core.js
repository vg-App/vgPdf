...
import {
  parseColor as utils_parseColor,
  support,
...
var vg_PSEUDO_ELEMENT = 'vg-PSEUDO-ELEMENT'
...
function closest(el, selector) {
  ...
  // https://github.com/telerik/vg/issues/6721
...
var cloneNodes = (function($) {
  if ($) {
    // if we have vg and jQuery, use this version as it will
    // maintain proper links between cloned element and vg
    // widgets (i.e. it clones jQuery data(), which isn't the same
    // as element's data attributes).
    // https://github.com/telerik/vg-ui-core/issues/2750
    return function cloneNodes(el) {
      ...
      if (el.nodeType == 1 /* Element */) {
        ...
        if (/^canvas$/i.test(el.tagName)) {
          ...
          // into the DOM.  https://github.com/telerik/vg/issues/5409
    ...
    return function cloneNodes(el) {
      const clone = (function dive(node) {
        ...
        if (node._vgExportVisual) {
          clone._vgExportVisual = node._vgExportVisual
        }
      ...
      // re-draw canvases - https://github.com/telerik/vg/issues/4872
      ...
      // https://github.com/telerik/vg/issues/5409
      const orig = el.querySelectorAll('input, select, textarea, option')
      slice(clone.querySelectorAll('input, select, textarea, option')).forEach(
        (el, i) => {
...
})(typeof window !== 'undefined' && window.vg && window.vg.jQuery)
...
function drawDOM(element, options) {
  ...
  cacheImages(element, function() {
    ...
    var hasPaperSize =
      options && options.paperSize && options.paperSize != 'auto'
  ...
  function handlePageBreaks(
    ...
    var container = doc.createElement('vg-PDF-DOCUMENT')
    var adjust = 0

    // make sure <tfoot> elements are at the end (Grid widget
    // places TFOOT before TBODY, tricking our algorithm to
    // insert a page break right after the header).
    // https://github.com/telerik/vg/issues/4699
    ...
    slice(copy.querySelectorAll('ol')).forEach(function(ol) {
      slice(ol.children).forEach(function(li, index) {
        li.setAttribute('vg-split-index', index)
      })
    })
    ...
    function doPageBreak() {
      ...
      cacheImages(pages, function() {
        ...
        // (https://github.com/telerik/vg/issues/5399), so this function will wait
    ...
    function keepTogether(el) {
      if (
        ...
        matches(el, options.keepTogether) &&
        el.offsetHeight <= pageHeight - adjust
      ) {
        return true
      }
    ...
    function splitElement(element) {
      ...
      var bottomBorder = parseFloat(
        getPropertyValue(style, 'border-bottom-width')
      )
        ...
        if (el.nodeType == 1 /* Element */) {
          ...
          if (
            !/^(?:static|relative)$/.test(
              getPropertyValue(getComputedStyle(el), 'position')
            )
          ) {
            continue
          }
    ...
    function breakAtElement(el) {
      ...
      if (options.repeatHeaders) {
        ...
        // If we break page in a vg Grid, repeat its header.  This ugly hack is
    ...
    function makePage() {
      var page = doc.createElement('vg-PDF-PAGE')
      setCSS(page, {
        display: 'block',
        boxSizing: 'content-box',
    ...
    function fallsOnMargin(thing) {
      ...
      return box.height > available
        ? 3
        : box.top - top > available
        ? 1
        : box.bottom - top > available
        ? 2
        : 0
    }
    ...
    function preventBulletOnListItem(el) {
      ...
      // https://github.com/telerik/vg-ui-core/issues/2732
      var li = closest(el, 'li')
      if (li) {
        li.setAttribute('vg-no-bullet', '1')
...
var getFontURL = (function(cache) {
  return function(el) {
    ...
    if (!url) {
      var m
      if (
        (m = /url\((['"]?)([^'")]*?)\1\)\s+format\((['"]?)truetype\3\)/.exec(
          el
        ))
      ) {
        url = cache[el] = m[2]
...
function cacheImages(element, callback) {
  ...
  function dive(element) {
    ...
    parseBackgroundImage(
      getPropertyValue(getComputedStyle(element), 'background-image')
    ).forEach(function(bg) {
      if (bg.type == 'url') {
        add(bg.url)
      }
...
function pleaseSetPropertyValue(style, prop, value, important) {
  ...
  if (browser.webkit) {
    ...
    prop =
      'ms' +
      prop.replace(/(^|-)([a-z])/g, function(s, p1, p2) {
...
function getBorderRadius(style, side) {
  var r = getPropertyValue(style, 'border-' + side + '-radius')
    .split(/\s+/g)
    .map(parseFloat)
  if (r.length == 1) {
    r.push(r[0])
...
function evalPseudoElementContent(element, content) {
  ...
  a.forEach(function(el) {
    ...
    if ((m = /^\s*(["'])(.*)\1\s*$/.exec(el))) {
      result.push(
        m[2].replace(/\\([0-9a-f]{4})/gi, function(s, p) {
...
function _renderWithPseudoElements(element, group) {
  if (element.tagName == vg_PSEUDO_ELEMENT) {
    _renderElement(element, group)
    return
  }
  let fake = []
  function pseudo(kind, place) {
    let style = getComputedStyle(element, kind),
    ...
    if (
      ...
      let psel = element.ownerDocument.createElement(vg_PSEUDO_ELEMENT)
  ...
  if (fake.length > 0) {
    ...
    element.className += ' vg-pdf-hide-pseudo-elements'
    _renderElement(element, group)
...
function _renderElement(element, group) {
  ...
  var backgroundImage = parseBackgroundImage(
    getPropertyValue(style, 'background-image')
  )
  var backgroundRepeat = splitProperty(
    getPropertyValue(style, 'background-repeat')
  )
  var backgroundPosition = splitProperty(
    getPropertyValue(style, 'background-position')
  )
  var backgroundOrigin = splitProperty(
    getPropertyValue(style, 'background-origin')
  )
  ...
  ;(function() {
    ...
    if (m) {
      ...
      var right =
        a[1] == 'auto' ? innerbox.right : parseFloat(a[1]) + innerbox.left
      var bottom =
        a[2] == 'auto' ? innerbox.bottom : parseFloat(a[2]) + innerbox.top
      var left =
        a[3] == 'auto' ? innerbox.left : parseFloat(a[3]) + innerbox.left
      var tmp = new Group()
  ...
  if (display == 'table-row') {
    ...
    // previously rendered cell.  https://github.com/telerik/vg/issues/4881
    boxes = []
  ...
  // images inside.  https://github.com/telerik/vg-ui-core/issues/3359
  if (
    element.tagName == 'A' &&
    element.href &&
  ...
  if (
    boxes.length > 0 &&
    display == 'list-item' &&
    !element.getAttribute('vg-no-bullet')
  ) {
    drawBullet(boxes[0])
  }
  ...
  ;(function() {
    ...
    if (isFormField(element)) {
      ...
      /^(hidden|auto|scroll)/.test(getPropertyValue(style, 'overflow'))
    ) {
      clipit()
    } else if (
      /^(hidden|auto|scroll)/.test(getPropertyValue(style, 'overflow-x'))
    ) {
      clipit()
    } else if (
      /^(hidden|auto|scroll)/.test(getPropertyValue(style, 'overflow-y'))
    ) {
      clipit()
    }
  })()
  ...
  function adjustBoxes(boxes) {
    if (/^td$/i.test(element.tagName)) {
      ...
      if (
        table &&
        getPropertyValue(table.style, 'border-collapse') == 'collapse'
      ) {
        ...
        if (
          firstCellBox.top == tableBox.top ||
          firstCellBox.left == tableBox.left
        ) {
  ...
  function drawEdge(color, len, Wtop, Wleft, Wright, rl, rr, transform) {
    ...
    edge.append(path)
    path
      .moveTo(rl.x ? Math.max(rl.x, Wleft) : 0, 0)
  ...
  function drawOneBackground(
    ...
    if (background.type == 'url') {
      ...
      if (img && img.width > 0 && img.height > 0) {
        drawBackgroundImage(group, box, img.width, img.height, function(
          group,
          rect
        ) {
    ...
    function drawBackgroundImage(group, box, img_width, img_height, renderBG) {
      ...
      // https://github.com/telerik/vg-ui-core/issues/2786
      //
      ...
      var rect = new geo.Rect(
        [orgBox.left + pos[0], orgBox.top + pos[1]],
  ...
  function drawBullet() {
    ...
    function _drawBullet(f) {
      saveStyle(element, function() {
        ...
        var bullet = element.ownerDocument.createElement(vg_PSEUDO_ELEMENT)
    ...
    function elementIndex(f) {
      ...
      var k = element.getAttribute('vg-split-index')
      if (k != null) {
  ...
  function drawOneBox(box, isFirst, isLast) {
    ...
    var shouldDrawLeft =
      left.width > 0 && ((isFirst && dir == 'ltr') || (isLast && dir == 'rtl'))
    var shouldDrawRight =
      right.width > 0 && ((isLast && dir == 'ltr') || (isFirst && dir == 'rtl'))
        ...
        var stops = gradient.stops.map(function(s, i) {
          ...
          if (offset != null) {
            ...
            implicit.forEach(function(s, i) {
              var stop = s.stop
              stop.offset =
                s.left + ((right - s.left) * (i + 1)) / (implicit.length + 1)
            })
            implicit = []
          } else {
...
function maybeRenderWidget(element, group) {
  ...
  if (element._vgExportVisual) {
    visual = element._vgExportVisual()
  } else if (
    window.vg &&
    window.vg.jQuery &&
    element.getAttribute(window.vg.attr('role'))
  ) {
    var widget = window.vg.widgetInstance(window.vg.jQuery(element))
  ...
  if (element.type == 'checkbox') {
    ...
    if (element.checked) {
      ...
      group.append(
        new Path()
          .stroke(color, 1.2)
          .moveTo(box.left + 0.22 * box.width, box.top + 0.55 * box.height)
          .lineTo(box.left + 0.45 * box.width, box.top + 0.75 * box.height)
          .lineTo(box.left + 0.78 * box.width, box.top + 0.22 * box.width)
      )
    }
  } else {
    group.append(
...
function renderFormField(element, group) {
  ...
  var doc = element.ownerDocument
  var el = doc.createElement(vg_PSEUDO_ELEMENT)
  var option
  ...
  if (tag == 'select') {
    if (element.multiple) {
      for (var i = 0; i < element.options.length; ++i) {
        option = doc.createElement(vg_PSEUDO_ELEMENT)
...
function renderText(element, node, group) {
  ...
  //
  // https://github.com/telerik/vg/issues/5232
  ...
  var estimateLineLength =
    (element.getBoundingClientRect().width / fontSize) * 5
  if (estimateLineLength === 0) {
    estimateLineLength = 500
  }

  // we'll maintain this so we can workaround bugs in Chrome's Range.getClientRects
  // https://github.com/telerik/vg/issues/5740
  ...
  function actuallyGetRangeBoundingRect(range) {
    ...
    if (microsoft || browser.chrome) {
      ...
      // one.  https://github.com/telerik/vg/issues/4674

      // Actually Chrome 50 got worse, since the rectangles can now have the width of a
      // full character, making it hard to tell whether it's a bogus rectangle or valid
      // selection location.  The workaround is to ignore rectangles that fall on the
      // previous line.  https://github.com/telerik/vg/issues/5740
  ...
  function drawText(str, box) {
    ...
    var text = new TextRect(
      str,
      new geo.Rect([box.left, box.top], [box.width, box.height]),
      {
        font: font,
        fill: { color: color }
      }
...
function renderElement(element, container) {
  ...
  if (
    /^(style|script|link|meta|iframe|svg|col|colgroup)$/i.test(element.tagName)
  ) {
    return
  }
